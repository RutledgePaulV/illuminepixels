...
(defn navigation [panel]
  [:nav
   [:div.nav-container
    [:div.nav-logo
     [:a {:href (routes/view->path :home-panel)} "home"]]
    [:ul.nav-links
     [:li (if (#{:blog-panel :blogs-panel} panel)
            [:a.active {:href (routes/view->path :blogs-panel)} [:h6 "blog"]]
            [:a {:href (routes/view->path :blogs-panel)} [:h6 "blog"]])]
     [:li (if (#{:game-panel :games-panel} panel)
            [:a.active {:href (routes/view->path :games-panel)} [:h6 "games"]]
            [:a {:href (routes/view->path :games-panel)} [:h6 "games"]])]
     [:li (if (#{:about-panel} panel)
            [:a.active {:href (routes/view->path :about-panel)} [:h6 "about"]]
            [:a {:href (routes/view->path :about-panel)} [:h6 "about"]])]]]])
