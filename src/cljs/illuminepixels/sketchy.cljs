...
(defn draw-state [drawing-state]
  (q/background 220)
  (doseq [{:keys [x y radius color]} (:circles drawing-state [])]
    (apply q/fill color)
  ...
  ([type slug dimensions framerate]
   (letfn [(setup []
           ...
           (mouse-press [{:keys [type slug] :as state} event]
             (when (and (<= 0 (:x event) (first dimensions))
                        (<= 0 (:y event) (second dimensions)))
               (let [message {:kind :mouse-pressed :type type :slug slug :event event}]
                 (rf/dispatch [::wfx/push :server message])))
             state)
